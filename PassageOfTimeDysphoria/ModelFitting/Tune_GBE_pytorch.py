...
def run_model(lte, outcomeAmount, mood_rating, time, pen_betaE, pen_betaA, pen_betaT,
    ...
    if parameters is None:
        ...
        # betaT initialized to Normal(0, 1)
        par_betaT = pt.randn(n_subjects, n_models, device=device, dtype=dtype, requires_grad=True)
        parameters = [par_M0, par_lambda, par_betaE, par_betaA, par_betaT]
    par_M0, par_lambda, par_betaE, par_betaA, par_betaT = parameters
    ...
    for iter_no in range(n_iterations):
        ...
        for trial_no in range(n_trials):
            ...
            T_new = pt.tensor(time[trial_no, :].reshape(n_subjects, 1), device=device, dtype=dtype)
            mood_new = m0 + pbA * sum_A_new + pbE * sum_E_new + par_betaT * T_new
        ...
        pen_const_betaT = pt.tensor(pen_betaT.reshape((1, -1)), device=device, dtype=dtype)

        # compute regularization terms
        reg_pen = pt.sum(pen_const_betaE * par_betaE**2) + \
                   pt.sum(pen_const_betaA * par_betaA**2) + \
                   pt.sum(pen_const_betaT * par_betaT**2)
...
def LoadDataAndRunModel(data_file='Mmi-gbeExplore_TrialForMdls.csv', n_subjects=5000, n_trials = 30,
                        pen_betaEA='tune', pen_betaT='tune',
    ...
    # Make "grid" of all combinations of E/A and T coefficients
    grid = 10.** np.array([(i,j) for i in np.linspace(-4,3,20) for j in np.linspace(-4,3,20)])
    ...
    pars, losses, mood_pred, obj_fn, test_loss = run_model(lte=lte, outcomeAmount=outcomeAmount, mood_rating=mood_rating, time=time,
              pen_betaE = grid[:,0], pen_betaA = grid[:,0], pen_betaT = grid[:,1],
    ...
    # get best parameters
    parnames = ['m0', 'lambda', 'beta_E', 'beta_A', 'beta_T']
...
parser.add_argument('-pt','--pen_betaT', default='tune', help='regularization penalty on beta_T parameter')
...
if __name__ == '__main__':
    ...
    if args.pen_betaT != 'tune':
        args.pen_betaT = float(args.pen_betaT)

    # Call main function
    LoadDataAndRunModel(data_file = args.data_file, n_subjects = int(args.n_subjects), n_trials = int(args.n_trials),
                        pen_betaEA = args.pen_betaEA, pen_betaT = args.pen_betaT,
